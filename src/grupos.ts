...
const ESCOPOS_GRUPO: readonly GrupoEscopo[] = [
  "GERAL",
  "IGREJA",
  "MINISTERIO",
  "MINISTERIO_LOCAL",
...
const ESCOPOS_SEM_IGREJA_UNICA: readonly GrupoEscopo[] = ["GERAL", "MINISTERIO", "PILAR"];

function isEscopoGrupo(value: string): value is GrupoEscopo {
  return (ESCOPOS_GRUPO as readonly string[]).includes(value);
}
 ...
 * vira `undefined`.
 */
function readPilarId(value: unknown): number | null | undefined {
  if (value === null) return null;
 ...
 * (número).
 */
export function parseGruposDetalhe(raw: unknown): GrupoDetalhe[] {
  if (!Array.isArray(raw)) return [];
  const out: GrupoDetalhe[] = [];
  ...
  for (const item of raw) {
    ...
    if (typeof o.escopo !== "string") continue;
    const escopo = o.escopo.trim().toUpperCase();
    if (!isEscopoGrupo(escopo)) continue;
    ...
    if ((escopo === "IGREJA" || escopo === "MINISTERIO_LOCAL") && !coreChurchId) continue;
    if ((escopo === "MINISTERIO" || escopo === "MINISTERIO_LOCAL") && !temaCodigo) continue;
    if (escopo === "PILAR" && (pilarId === undefined || pilarId === null)) continue;

    const detalhe: GrupoDetalhe = { slug, escopo };
    if (coreChurchId !== undefined) detalhe.coreChurchId = coreChurchId;
    if (temaCodigo !== undefined) detalhe.temaCodigo = temaCodigo;
    if (pilarId !== undefined) detalhe.pilarId = pilarId;
    out.push(detalhe);
  }
 ...
 */
export function escopoAlcancaIgreja(d: GrupoDetalhe, coreChurchId: string): boolean {
  if (!d) return false;
  if (ESCOPOS_SEM_IGREJA_UNICA.includes(d.escopo)) return true;
  return Boolean(coreChurchId) && d.coreChurchId === coreChurchId;
}

/** Filtra da lista os grupos cujo alcance cobre a igreja dada. */
export function gruposQueAlcancamIgreja(
  list: GrupoDetalhe[],
  coreChurchId: string,
): GrupoDetalhe[] {
...
export function gruposDoMinisterio(list: GrupoDetalhe[], temaCodigo: string): GrupoDetalhe[] {
  return (Array.isArray(list) ? list : []).filter(
    (d) => (d.escopo === "MINISTERIO" || d.escopo === "MINISTERIO_LOCAL") && d.temaCodigo === temaCodigo,
  );
}
 ...
 */
export function gruposDoPilar(list: GrupoDetalhe[], pilarId: number): GrupoDetalhe[] {
  return (Array.isArray(list) ? list : []).filter((d) => {
    if (d.escopo === "PILAR") return d.pilarId === pilarId;
 ...
 */
export function pilarDoTema(temaCodigo: string): number | null {
  if (typeof temaCodigo !== "string") return null;
  const prefixo = temaCodigo.trim().split(".")[0];
  if (!prefixo || !/^\d+$/.test(prefixo)) return null;
