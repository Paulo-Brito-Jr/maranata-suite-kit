 ...
 */
export function parseDataHoraLocalBR(valor: string): Date {
  const s = valor.trim();
  if (TZ_SUFFIX_RE.test(s)) return new Date(s);
  if (DATE_ONLY_RE.test(s)) return new Date(`${s}T00:00:00${BRT_OFFSET}`);
...
function partesBRT(instante: Date): PartesBRT {
  const p = PARTS_FMT.formatToParts(instante).reduce<Record<string, string>>((acc, x) => {
...
export function formatarDataBR(d: Date | string | number): string {
  const b = partesBRT(comoInstante(d));
  return `${String(b.dia).padStart(2, "0")}/${String(b.mes).padStart(2, "0")}/${b.ano}`;
}

/** "dd/mm/aaaa HH:mm" no horário de Brasília. */
export function formatarDataHoraBR(d: Date | string | number): string {
  const b = partesBRT(comoInstante(d));
